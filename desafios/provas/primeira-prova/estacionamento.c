...
int remove_Pilha_manobrista(Pilha* pi, Lista_func* li, int placa_posicao, struct carro car, struct funcionario func){
    ...
    //Retirando elemento excluido da pilha QUALQUER COISA MOVER ISSO PARA BAIXO
    // Pilha* p2 = pi;
    // p2 = (Pilha*)realloc(pi, pi->qtd * sizeof(Pilha));
    // if (p2 != NULL)
    // {
    //     pi = p2;
    // } else {
...
int remove_Lista_manobrista(Lista_carro* li_carro, Lista_func* li_func, int placa_posicao, struct carro_lista car, struct funcionario func, int var ){
    ...
    if(li_carro == NULL)
        return 0;
    //printf("EU RECEBO PRIMEIRO DE TUDO: %d\n", li_carro->qtd);
...
//         return 0;
//     //printf("EU RECEBO PRIMEIRO DE TUDO: %d\n", li_carro->qtd);
//     while (li_carro->qtd != placa_posicao && placa_posicao == MAX)
//     {        
//         //Quando todos os manobristas tiverem tirado um carro, reinicio a sequencia
//         if (li_func->qtd == 0)
//         {
...
int main(){
    ...
    int i, j, k, x = 0, qtd = 0, aux = 0, z = 1, hora = 1, var = 2, y;
    // char ch = 'A';
    float acum = 0, acum2 = 0;   

    srand(time(NULL));
    
    //Preenchendo carro de forma random
    // for (int i = 0; i < MAX; i++)
    // {
    //     car[i].name = 'A';
    //     car[i].name = car[i].name + (char)(rand() % 26);
    //     car[i].placa = 9000 + (rand() % 9999);
    //     // car[i].horas = 1 + (rand() % hora);
    //     // hora = car[i].horas;    
    //     car[i].horas = 5;
    //     insere_Pilha(pi,car[i]);
    // }
    // printf("Estacionamento lotado!\n");
    // //Preenchendo funcionarios de forma random
    // for (j = 0; j < N; j++)
    // {        
    ...
    // }
    // insertion_sort(li, fun[N]);
    // //Sorteio um número
    // // int y = 0 + (rand() % MAX);
    // //TESTE ESTÁ DANDO ERRADO COM y= 2 y--;
    // int y = 0;
    // for (k = 0; k < MAX; k++)
    // {                      
    ...
    //         remove_Pilha_manobrista(pi,li, y);
